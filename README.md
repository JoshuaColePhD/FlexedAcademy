...
Turn a weekly request into a structured, standards-aligned lesson plan — with the
source behind every standard it cites — and export it as a district-formatted Word document.
...
<sub>The walkthrough is shown as a GIF because GitHub only renders inline video players for files uploaded through its own UI. Full-quality video: [MP4](docs/recruiter/FlexedAcademy_Walkthrough.mp4) · [WebM](docs/recruiter/FlexedAcademy_Walkthrough.webm). On the live site, click **Explore demo (read-only)** on the sign-in page — no setup required.</sub>
...
| | |
| --- | --- |
| **Plans a full week** | Generates a five-day, standards-aligned plan from a teacher's prompt and class context. |
| **Grounded in real standards** | Cites standards from source documents — not model memory — with verbatim text and source metadata. |
| **Shows its work** | Surfaces the cited standards, their sources, and grounding warnings for teacher review. |
| **Matches your district** | Renders into school-specific lesson-plan templates and exports DOCX (and QTI for quizzes). |
| **Conversational** | Supports coaching, day-level revision, quiz generation, and teacher-owned pacing guides. |
