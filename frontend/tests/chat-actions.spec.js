...
test('idea suggestions appear as a choice box above the composer', async ({ page }) => {
  ...
  const option = dock.getByRole('button', { name: 'Quadratic functions in vertex form' })
  const rest = await option.evaluate((el) => getComputedStyle(el).backgroundColor)
  await option.hover()
  await expect.poll(async () => option.evaluate((el) => getComputedStyle(el).backgroundColor)).not.toBe(rest)
