  ...
  return (
    <div className={`split-layout-shell${isGeneralLayout ? ' split-layout-shell-general' : ''} flex h-full min-h-0 w-full overflow-hidden bg-paper/30 backdrop-blur-3xl saturate-[1.2] border border-white/5`}>
          ...
          <nav className="flex flex-col gap-0.5 px-2">
            {tabs.map((tab) => (
              <button
                ...
                className={`flex min-h-10 items-center gap-2 rounded-lg px-2 text-sm transition-colors text-left ${
                  activeTab === tab.id
                    ? 'bg-paper shadow-sm ring-1 ring-black/5 font-medium text-ink'
                    : 'text-ink-soft hover:bg-paper-inset hover:text-ink'
                }`}
              >
                {tab.icon ? <tab.icon size={15} aria-hidden="true" className="shrink-0 text-ink-muted" /> : null}
      ...
      <div className="split-layout-content flex-1 overflow-y-auto relative" ref={scrollContainerRef}>
        <div className={`split-layout-content-inner mx-auto px-5 py-5 md:px-8 md:py-6 pb-16 ${contentMaxWidth}`}>
          {isGeneralLayout ? (
            <>
              ...
              {tabs.length ? (
                <nav className="split-layout-general-tabs" aria-label={`${title} sections`}>
                  {tabs.map((tab) => (
                    <button
                      key={tab.id}
                      type="button"
