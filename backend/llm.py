...
the wrong shape instead of as one or two dropdown values.
...
def generate_quiz(
    ...
    *,
    ...
    if instruction:
        system_prompt += "\n\nTEACHER REQUEST AND ESTABLISHED CONSTRAINTS:\n" + instruction
    custom_instructions = custom_instructions_for(user_id)
...
def generate_quiz_tool_payload(args: dict) -> dict:
    ...
    return {
        ...
        "num_questions": args.get("num_questions") or 5,
...
def revise_quiz(
    user_id: str, plan: dict | None, existing_quiz: dict, feedback: str, *, class_id: str | None = None, skip_cache: bool = False, question_indices: list[int] | None = None
) -> dict:
    ...
    if question_indices:
        preserve_quiz_scope(existing_quiz, existing_quiz, question_indices)
        system_prompt += ("\n\nChange ONLY question numbers " + ", ".join(str(i + 1) for i in question_indices)
                          + ". Return the full quiz in its original order and length. Preserve passages, title, and all other questions.")
    custom_instructions = custom_instructions_for(user_id)
    if custom_instructions:
        system_prompt += (
            "\n\nTEACHER'S GLOBAL CUSTOM INSTRUCTIONS — style/format preferences only, "
    ...
    return preserve_quiz_scope(existing_quiz, _randomize_mc_choice_order(loads_lenient(content or "")), question_indices)
...
def rewrite_day(
    ...
    engagement_strategy with an uncontrolled number of choices while the
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "for a quiz or test, set also_quiz true so this turn produces both."
            ),
        },
    },
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "downloadable file and is not also asking for a week in this message. If they asked for "
                "a week and a quiz together, call generate_lesson_plan with also_quiz true instead. "
                "Never volunteer a quiz they did not ask for, and never offer assessment design as a "
                "separate job. Prefer putting checks and exit tickets in the week's plan. Use named question "
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "do not treat a pacing-guide unit as already confirmed. For a new weekly lesson plan, "
