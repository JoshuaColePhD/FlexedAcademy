...
# Generated by the ingest scripts (data/processed/ is gitignored — see that
# file's comment) and too large to track in git: alcos_chunks.json alone is
# 29MB, the entire K-12 Alabama Course of Study corpus. Render's build never
# runs ingestion (render.yaml's buildCommand is just the frontend build + pip
# install), so without this, a fresh deploy has NONE of these locally — for a
# while git tracked chunks.json and ap_chunks.json as a special case, but that
# still silently limited production to AP-only, grades 9-12, since
# alcos/pre_ap/act were gitignored with no exception and no error to explain
# the gap. Durable copies of all five now live in Supabase Storage (uploaded
# by scripts/upload_chunks_to_storage.py after each re-ingest); restored here
# on demand the same way backend/db.py self-heals a missing QTI file.
_UNTRACKED_CHUNK_FILES = (
    "chunks.json",
    "ap_chunks.json",
    "alcos_chunks.json",
    "pre_ap_chunks.json",
