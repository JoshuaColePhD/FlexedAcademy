...
async def lifespan(app: FastAPI):
    ...
    # Keep the standards indexes lazy. Each index walks the full standards
    # corpus and the three old background warmers were dispatched at the same
    # time. On Render's 512MB instance that meant several copies of the corpus
    # could be materializing during boot, before the health check had even
    # settled. The underlying `load_chunks()` cache is shared, so the first
    # standards request still pays this cost once, but it does so under normal
    # request back-pressure instead of creating an unbounded startup spike.
    loop = asyncio.get_running_loop()
