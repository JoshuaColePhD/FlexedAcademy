...
- `chat_turn_policy()` describes a turn rather than gating it: `tools_enabled` (always true for typed chat), `command_surface`, `pending_intent`, and `plan_context`. `pending_intent` reads the exchange, not the last message, so an answer to a clarifying question or a "yes" to an offer completes the request that prompted it.
